...
class Client extends EventEmitter {
    ...
    */
    ...
    /**
     * Create a automod rule in a guild
     * @arg {String} guildID The ID of the guild to create the automod rule
     * @arg {String} name The rule name
     * @arg {Object} options Options for the rule
     * @arg {Number} [options.eventType] The event type
     * @arg {Number} [options.triggerType] The trigger type
     * @arg {Array<String>} [options.keywordFilter] Substrings which will be searched for in content
     * @arg {Array<Number>} [options.presets] The internally pre-defined wordsets which will be searched for in content
     * @arg {Array<String>} [options.allowList] Substrings which will be exempt from triggering the preset trigger type
     * @arg {Array<Object>} [options.actions] The actions which will execute when the rule is triggered
     * @arg {Boolean} [options.enabled] Whether the rule is enabled. Default: false
     * @arg {Array<String>} [options.exemptRoles] The role ids that should not be affected by the rule (Maximum of 20)
     * @arg {Array<String>} [options.exemptChannels] The channel ids that should not be affected by the rule (Maximum of 50)
     * @returns {Promise<AutomodRule>}
     */
    createAutomodRule(guildID, name, options, reason) {
        const trigger_metadata = null;
        if(options.keywordFilter !== undefined) {
            Object.assign(Object.create(trigger_metadata), {keyword_filter: options.keywordFilter});
        }
        if(options.presets !== undefined) {
            Object.assign(Object.create(trigger_metadata), {presets: options.presets});
        }
        if(options.allowList !== undefined) {
            Object.assign(Object.create(trigger_metadata), {allow_list: options.allowList});
        }
        return this.requestHandler.request("POST", Endpoints.GUILD_AUTOMOD_RULES(guildID), true, {
            name: name,
            event_type: options.eventType,
            trigger_type: options.triggerType,
            trigger_metadata: trigger_metadata || undefined,
            actions: options.actions,
            enabled: !!options.enabled,
            exempt_roles: options.exemptRoles,
            exempt_channels: options.exemptChannels,
            reason: reason
        }).then((rule) => new AutomodRule(rule, this));
    }
    ...
    */
    ...
    createGuildScheduledEvent(guildID, options, reason) {
        return this.requestHandler.request("POST", Endpoints.GUILD_SCHEDULED_EVENT(guildID), true, {
            ...
            image: options.image,
            reason: reason
        }).then((event) => new GuildScheduledEvent(event, this));
    }
    /**
    ...
    * @arg {String} [options.data.components[].custom_id] The ID of the component (type 2 style 0-4, and type 4 only)
    ...
    * @arg {String} [options.data.components[].placeholder] The placeholder text for the component when no option is selected (type 3 and 4)
    * @arg {Number} [options.data.components[].style] The style of the component (type 2 only) - For buttons, if 0-4, `custom_id` is required; if 5, `url` is required, for text inputs 1-2
    ...
    */
     ...
     * @arg {String} guildID The ID of the guild
     * @arg {String} ruleID The ID of the rule
     * @arg {String} [reason] The reason to be displayed in audit logs
     * @returns {Promise}
     */
    deleteAutomodRule(guildID, ruleID, reason) {
        return this.requestHandler.request("DELETE", Endpoints.GUILD_AUTOMOD_RULES(guildID, ruleID), true, {reason});
    }
    ...
    * @arg {String} guildID The ID of the guild
    * @arg {String} eventID The ID of the scheduled event
    * @returns {Promise}
    */
    ...
    */
    ...
    /**
     * Edit a automod rule in a guild
     * @arg {String} guildID The ID of the guild
     * @arg {String} ruleID The ID of the rule
     * @arg {Object} options Options for the rule
     * @arg {String} [options.name] The rule name
     * @arg {Number} [options.eventType] The event type
     * @arg {Number} [options.triggerType] The trigger type
     * @arg {Array<String>} [options.keywordFilter] Substrings which will be searched for in content
     * @arg {Array<Number>} [options.presets] The internally pre-defined wordsets which will be searched for in content
     ...
     * @returns {Promise<AutomodRule>}
     */
    editAutomodRule(guildID, ruleID, options, reason) {
        const trigger_metadata = null;
        if(options.keywordFilter !== undefined) {
            Object.assign(Object.create(trigger_metadata), {keyword_filter: options.keywordFilter});
        }
        ...
        return this.requestHandler.request("PATCH", Endpoints.GUILD_AUTOMOD_RULES(guildID, ruleID), true, {
            name: options.name,
            event_type: options.eventType,
            trigger_type: options.triggerType,
            trigger_metadata: trigger_metadata || undefined,
    ...
    */
     ...
     * @returns {Promise<GuildScheduledEvent>}
     */
    editGuildScheduledEvent(guildID, eventID, options, reason) {
    ...
    */
    ...
    /**
     * Get an specific automod rule
     * @arg {String} guildID The ID of the guild
     * @arg {String} ruleID The ID of the rule
     * @returns {Promise<AutomodRule>}
     */
    getAutomodRule(guildID, ruleID) {
        return this.requestHandler.request("GET", Endpoints.GUILD_AUTOMOD_RULES(guildID, ruleID), true).then((rule) => new AutomodRule(rule, this));
    }

    /**
     * Get all configured automod rules
     * @arg {String} guildID The ID of the guild
     * @arg {Boolean} syncCache If true will automatically sync the bot automod rules cache
     * @returns {Promise<Array<AutomodRule>>}
     */
    getAutomodRules(guildID, syncCache = true) {
        return this.requestHandler.request("GET", Endpoints.GUILD_AUTOMOD_RULES(guildID), true).then((response) => {
            if(syncCache && this.guilds.has(guildID)) {
                const guild = this.guilds.get(guildID);
                guild.automodRules.clear();
                response.forEach((rule) => guild.automodRules.add(rule));
            }
            return response.map((r) => new AutomodRule(r, this));
        });
    }
    ...
    */
    ...
    getGuildScheduledEvent(guildID, eventID) {
        return this.requestHandler.request("GET", Endpoints.GUILD_SCHEDULED_EVENT(guildID, eventID) + "?with_user_count=true", true).then((event) => new GuildScheduledEvent(event, this));
    }
    /**
     * Get a guild scheduled events
     * @arg {String} guildID The ID of the guild
     * @returns {Promise<Array<GuildScheduledEvent>>}
     */
    getGuildScheduledEvents(guildID) {
        return this.requestHandler.request("GET", Endpoints.GUILD_SCHEDULED_EVENT(guildID) + "?with_user_count=true", true).then((events) => events.map((e) => new GuildScheduledEvent(e, this)));
    }
    /**
     * Get all subscribed users for a scheduled event
     * @arg {String} guildID The ID of the guild
     * @arg {String} eventID The ID of the scheduled event
     * @arg {Object} options
     ...
     * @returns {Promise<Array<Object>>}
     */
    getGuildScheduledEventUsers(guildID, eventID, {limit = 100, before = null, after = null} = {}) {
        ...
        return this.requestHandler.request("GET", Endpoints.GUILD_SCHEDULED_EVENT(guildID, eventID) + `/users?${params.toString()}`)
            .then((data) => data.map(({user, member}) => {
                member.user = user;
                return {
    ...
    */
    getOAuthApplication() {
        return this.requestHandler.request("GET", Endpoints.OAUTH2_APPLICATION("@me"), true);
    }
    ...
    */
    getRESTGuild(guildID, withCounts = false) {
        ...
        return this.requestHandler.request("GET", Endpoints.GUILD(guildID), true, {
            with_counts: withCounts
        }).then((guild) => this.guilds.add(guild, this));
    }
    ...
    */
    getRESTGuildMember(guildID, memberID) {
        ...
        return this.requestHandler.request("GET", Endpoints.GUILD_MEMBER(guildID, memberID), true).then((member) => {
            /**
             * @type {Guild}
             */
            const guild = this.guilds.get(guildID);

            return guild.members.add(new Member(member, this.guilds.get(guildID), this));
        });
    }
    ...
    */
    getRESTUser(userID) {
        ...
        return this.requestHandler.request("GET", Endpoints.USER(userID), true).then((user) => this.users.add(user, this));
    }
