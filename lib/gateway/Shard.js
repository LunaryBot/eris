    ...
    wsEvent(packet) {
        switch(packet.t) { /* eslint-disable no-redeclare */ // (╯°□°）╯︵ ┻━┻
            ...
            case "STAGE_INSTANCE_UPDATE": {
                const guild = this.client.guilds.get(packet.d.guild_id);
                if(!guild) {
                    this.emit("stageInstanceUpdate", new StageInstance(packet.d, this.client), null);
                    break;
                }
            ...
            case "GUILD_SCHEDULED_EVENT_CREATE": {
                 ...
                 * @prop {GuildScheduledEvent} Event The scheduled event that was created
                 */
                this.emit("guildScheduledEventCreate", guild.scheduledEvents.add(packet.d));
                break;
            }
            case "GUILD_SCHEDULED_EVENT_DELETE": {
                 ...
                 * @prop {GuildScheduledEvent} Event The scheduled event that was deleted
                 */
                this.emit("guildScheduledEventDelete", guild.scheduledEvents.remove(packet.d) || new GuildScheduledEvent(packet.d, this.client));
                break;
            }
            case "GUILD_SCHEDULED_EVENT_UPDATE": {
                const guild = this.client.guilds.get(packet.d.guild_id);
                let event = guild ? guild.scheduledEvents.get(packet.d.id) : null;
                let oldEvent = null;
                if(event) {
                    oldEvent = {
                        status: event.status,
                ...
                if(typeof oldEvent === "object" && oldEvent.status !== packet.d.status) {
                    switch(packet.d.status) {
                    /**
                     * Fired when a guild scheduled event is active
                     * @event Client#guildScheduledEventActive
                     * @prop {GuildScheduledEvent} event The scheduled event that was actived
                     */
                        case Constants.GuildScheduledEventsStatus.ACTIVE: this.emit("guildScheduledEventActive", event);
                            break;
                            /**
                     * Fired when a guild scheduled event is canceled
                     * @event Client#guildScheduledEventCanceled
                     * @prop {GuildScheduledEvent} event The scheduled event that was canceled
                     */
                        case Constants.GuildScheduledEventsStatus.CANCELED: this.emit("guildScheduledEventCanceled", event);
                            break;
                            /**
                     * Fired when a guild scheduled event is completed
                     * @event Client#guildScheduledEventCompleted
                     * @prop {GuildScheduledEvent} event The scheduled event that was completed
                     */
                        case Constants.GuildScheduledEventsStatus.COMPLETED: this.emit("guildScheduledEventCompleted", event);
                            break;
                            /**
                     * Fired when a guild event is scheduled
                     * @event Client#guildScheduledEvent
                     * @prop {GuildScheduledEvent} event The event that was scheduled
                     */
                        case Constants.GuildScheduledEventsStatus.SCHEDULED: this.emit("guildScheduledEvent", event);
                    }
                }
                 ...
                 * @prop {GuildScheduledEvent} Event The scheduled event that was updated
                 * @prop {Object?} Before The old state of the scheduled event
                 */
                this.emit("guildScheduledEventUpdate", event, oldEvent);
                break;
            }
            case "GUILD_SCHEDULED_EVENT_USER_ADD": {
                if(this.client.guilds.has(packet.d.guild_id)) {
                    const guild = this.client.guilds.get(packet.d.guild_id);
                    if(guild.scheduledEvents.has(packet.d.guild_scheduled_event_id)) {
                        guild.scheduledEvents.get(packet.d.guild_scheduled_event_id).userCount++;
                    }
                }
                /**
                 * Fired when someone subscribe for a guild scheduled event
                 * @event Client#guildScheduledEventUserAdd
                 * @prop {Object} Event
                 * @prop {String} Event.userID The ID of the user
                 * @prop {String} Event.guildID The ID of the guild
                 * @prop {String} Event.eventID The ID of the event
                 */
                this.emit("guildScheduledEventUserAdd", {userID: packet.d.user_id, guildID: packet.d.guild_id, eventID: packet.d.guild_scheduled_event_id});
                break;
            }
            case "GUILD_SCHEDULED_EVENT_USER_REMOVE": {
                ...
                /**
                 * Fired when someone unsubscribe for a guild scheduled event
                 ...
                 * @prop {String} Event.eventID The ID of the event
                 */
                this.emit("guildScheduledEventUserRemove", {userID: packet.d.user_id, guildID: packet.d.guild_id, eventID: packet.d.guild_scheduled_event_id});
                break;
            }
            case "AUTO_MODERATION_RULE_CREATE": {
                ...
                /**
                 * Fired when a automod rule is created
                 * @event Client#automodRuleCreate
                 * @prop {AutomodRule} Rule The automod rule that was created
                 */
                this.emit("automodRuleCreate", guild.automodRules.add(packet.d));
                break;
            }
            case "AUTO_MODERATION_RULE_DELETE": {
                const guild = this.client.guilds.get(packet.d.guild_id);
                if(!guild) {
                    this.emit("automodRuleDelete", new AutomodRule(packet.d, this.client));
                    break;
                }
                /**
                 * Fired when a automod rule is deleted
                 * @event Client#automodRuleDelete
                 * @prop {AutomodRule} Rule The automod rule that was deleted
                 */
                this.emit("automodRuleDelete", guild.automodRules.remove(packet.d) || new AutomodRule(packet.d, this.client));
                break;
            }
            case "AUTO_MODERATION_RULE_UPDATE": {
                const guild = this.client.guilds.get(packet.d.guild_id);
                let rule = guild ? guild.automodRules.get(packet.d.id) : null;
                let oldRule = null;
                if(rule) {
                    oldRule = {
                    ...
                    rule = guild.automodRules.update(packet.d);
                    if(oldRule.enabled !== packet.d.enabled) {
                        if(packet.d.enabled) {
                            /**
                             * Fired when an automod rule is enabled
                             * @event Client#automodRuleEnabled
                             * @prop {AutomodRule} Rule The automod rule that was enabled
                             */
                            this.emit("automodRuleEnabled", rule);
                        } else {
                            /**
                             * Fired when an automod rule is disabled
                             * @event Client#automodRuleDisabled
                             * @prop {AutomodRule} Rule The automod rule that was disabled
                             */
                            this.emit("automodRuleDisabled", rule);
                        }
                    }
                } else {
                    rule = new AutomodRule(packet.d, this.client);
                }
                /**
                 * Fired when an automod rule is updated
                 * @event Client#automodRuleUpdate
                 * @prop {AutomodRule} Rule The automod rule that was updated
                 * @prop {Object?} Before The automod rule object before the update
                 */
                this.emit("automodRuleUpdate", rule, oldRule);
                break;
            }
            case "AUTO_MODERATION_ACTION_EXECUTION": {
                const obj = {
                    guildID: packet.d.guild_id,
                    action: AutomodRule._formatAction(packet.d.action),
                ...
                /**
                 * Fired when an automod action has been executed.
                 * @event Client#automodActionExecution
                 * @prop {Object} Execution
                 * @prop {String} Execution.guildID The ID of the guild
                 * @prop {Object} Execution.action The action which was executed
                 * @prop {String} Execution.ruleID The ID of the rule which action belongs to
                 * @prop {Number} Execution.ruleTriggerType The trigger type of rule which was triggered
                 * @prop {String} Execution.userID The ID of the user which generated the content which triggered the rule
                 * @prop {String} Execution.channelID The ID of the channel in which user content was posted
                 * @prop {String?} Execution.messageID The ID of any user message which content belongs to
                 * @prop {String?} Execution.alertSystemMessageID The ID of any system auto moderation messages posted as a result of this action
                 * @prop {String?} Execution.content The user generated text content
                 * @prop {String?} Execution.matchedKeyword The word or phrase configured in the rule that triggered the rule
                 * @prop {String?} Execution.matchedContent The substring in content that triggered the rule
                 */
                this.emit("automodActionExecution", obj);
                break;
            }
            default: {
                /**
